...
public class GatewayAutoConfiguration{
     ...
     * 可直接通过 RouteLocator，编码形式构建 Route
     *
     * @param context
     * @return
     */
    ...
    @Bean
    public ConfigurationService gatewayConfigurationService(
    ...
    /**
     *
     * GatewayAutoConfiguration 进行自动创建。
     *
     * @param webHandler
     *            webHandler就是一个封装了Global Filter的对象
     * @param routeLocator
     *            routeLocator保存了所有Route的对象
    ...
    @Bean
    public WebsocketRoutingFilter websocketRoutingFilter(
    ...
    protected static class NettyConfiguration{
        ...
        public HttpClient gatewayHttpClient(HttpClientProperties properties){
            ...
            if (pool.getType() == DISABLED){
                ...
                connectionProvider = ConnectionProvider
                                .fixed(pool.getName(), pool.getMaxConnections(), pool.getAcquireTimeout(), pool.getMaxIdleTime());
            }else{
            ...
            if ((ssl.getKeyStore() != null && ssl.getKeyStore().length() > 0) || ssl.getTrustedX509CertificatesForTrustManager().length > 0
