...
public class RouteDefinitionRouteLocator implements RouteLocator,BeanFactoryAware,ApplicationEventPublisherAware{
    ...
    /**
     * routeDefinitionLocator 是一个 CompositeRouteDefinitionLocator 类型实例
     *
    ...
    @Deprecated
    public RouteDefinitionRouteLocator(RouteDefinitionLocator routeDefinitionLocator, List<RoutePredicateFactory> predicates,
                    List<GatewayFilterFactory> gatewayFilterFactories, GatewayProperties gatewayProperties, ConversionService conversionService){
    ...
    private void initFactories(List<RoutePredicateFactory> predicates){
        predicates.forEach(factory -> {
            ...
            if (this.predicates.containsKey(key)){
                this.logger.warn(
                                "A RoutePredicateFactory named " + key + " already exists, class: " + this.predicates.get(key)
    ...
    private Route convertToRoute(RouteDefinition routeDefinition){
        //解析Predicate，将 PredicateDefinition 转出 AsyncPredicate
    ...
    List<GatewayFilter> loadGatewayFilters(String id,List<FilterDefinition> filterDefinitions){
        ...
        for (int i = 0; i < filterDefinitions.size(); i++){
            ...
            if (logger.isDebugEnabled()){
                logger.debug("RouteDefinition " + id + " applying filter " + definition.getArgs() + " to " + definition.getName());
            }
    ...
    /**
     * 将FilterDefinition转成GatewayFilter,包括配置中定义的默认Filter和直接配置的Filter，不包括全局过滤器
     * 
